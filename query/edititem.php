...
if (empty($item_group_code)) {
	...
	$updateitemheader = $conn->query(" update tbl_item_code set icc_id = '$item_group_code' ,pl_id = '$price_list' ,date_use = '$date_use' ,ccy = '$ccy'  WHERE it_id='$item_id'    ");
	if ($updateitemheader) {
		...
		if ($delitemlist) {
			...
			for ($i = 0; $i < ($countbox); $i++) {
				...
				values ('$item_group_code','$item_id','$full_code','$last_num','$item_company_code','$itemcode[$i]','$itemname[$i]','$item_price[$i]',
