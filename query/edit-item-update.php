...
$insCourse = $conn->query(" delete from tbl_item_edit_detail_list where ie_id = '$ie_id'  ");
...
if ($insCourse) {
    ...
    for ($i = 0; $i < ($countbox); $i++) {
        ...
        $insertItem = $conn->query("  insert into tbl_item_edit_detail_list (ie_id,item_id,item_name,buy_unit,sale_unit,pack,weight,brand_name,date_register) 
				values ('$ie_id','$itemid[$i]','$itemname[$i]','$buy_unit[$i]','$sale_unit[$i]','$pack[$i]','$weight[$i]','$brand_name[$i]', now());  ");
